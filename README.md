...
- ログイン後は写真スライドショーの「トップ」ページを表示（`photos/`フォルダに写真を置くだけで反映）。次の予定・あなたの未回答件数もひと目で分かる
...
- メンバー管理は生年月日（年齢は自動計算して薄字表示）、電話番号（ハイフン必須）、衣装/Tシャツサイズ、担当を管理
...
| `photos/`（任意） | トップページのスライドショー用写真。`1.jpg`〜`12.jpg`という名前で置くだけで反映される（詳細は`photos/README.md`） |
