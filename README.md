...
- 予定作成時に日程調整リンクを自動発行。リンクが無くても「出欠回答」タブから予定を選んで回答できる
...
- メンバー管理は生年月日（年齢は自動計算して薄字表示）、電話番号（ハイフンは無くても可）、袴/Tシャツサイズ、担当を管理
...
1. 管理者が「管理 > 予定一覧」の「予定を追加」またはその場の「編集」から予定を保存する（政・スタッフは上部タブの「予定追加」から。予定ごとの日程調整リンクが自動発行される）
2. 共有したい場合は予定カード・管理画面から共有文をコピーして送る
3. メンバーはリンクから開くか、ログイン後に「出欠回答」タブで予定を選んで回答する
...
日程アンケート（候補日程調整）も同様に、管理画面の一覧から共有リンク・共有文を発行できます（`?poll=...` 形式）。リンクを開くとログイン後にその日程アンケートの画面へ直接移動します。
