...
// スタッフは「予定一覧」「予定追加」しか使わないので、その2つに絞った説明を
// 表示する（出欠回答・日程アンケート・留意点はスタッフには関係が無いため省略）。
function renderGuideView() {
  const el = document.getElementById('guideContent');
  if (!el) return;
  el.innerHTML = isStaff() ? staffGuideHtml() : memberGuideHtml();
}

function staffGuideHtml() {
  return `
    <section class="panel">
      <h2>使い方・見方</h2>
      ...
      <div class="panel-inset">
        <h3>予定一覧</h3>
        <ul class="guide-list">
          <li>若衆全体の予定が日付ごとに並びます。予定名の左の帯の色で分類（本祭・準備・会議など）が分かります。</li>
          <li>見出しをタップすると詳細が開き、各メンバーの出欠状況（参加／不参加／未定／時間限定／未回答）を確認できます。</li>
          <li>絞り込み（分類・年月・回答状況など）で見たい予定だけを表示できます。</li>
          <li>この画面はメンバーの出欠状況を確認するためのもので、スタッフがここから代理で出欠を入力・変更することはできません（本人か管理者のみ変更できます）。</li>
        </ul>
      </div>
...
function memberGuideHtml() {
  return `
    <section class="panel">
      <h2>使い方・見方</h2>
      <p class="muted">各画面の使い方と、若衆として使ううえで気をつけてほしい点をまとめています。</p>

      <div class="panel-inset">
        <h3>予定一覧</h3>
        <ul class="guide-list">
          <li>若衆全体の予定が日付ごとに並びます。予定名の左の帯の色で分類（本祭・準備・会議など）が分かります。</li>
          <li>見出しをタップすると詳細が開閉します。自分がまだ回答していない「要回答」の予定は最初から開いた状態で表示されます。</li>
          <li>自分の名前の横にある「参加／不参加／未定／時間限定」ボタンをタップすると、その場で回答できます。不参加・未定・時間限定を選ぶと、理由やいつまでに分かるかなどの詳細欄が自動で開きます。</li>
          <li>タップしただけではまだ保存されていません（未保存の印がつきます）。画面右下の「回答完了」ボタンを押して初めてサイトに反映されます。押す前に画面を閉じると入力が消えるので注意してください。</li>
        </ul>
      </div>
      ...
      <div class="panel-inset">
        <h3>出欠回答</h3>
        <ul class="guide-list">
          <li>予定一覧でもその場で回答できますが、共有された回答リンク（LINE等で送られてくる個別のURL）から開いた場合は、この専用画面が開きます。入力できる項目は予定一覧のその場回答と同じです。</li>
          <li>回答期限が設定されている予定は、期限日を過ぎると自分では回答を変更できなくなります（期限日当日23:59までは変更可能）。</li>
        </ul>
      </div>
      ...
      <div class="panel-inset">
        <h3>日程アンケート</h3>
        <ul class="guide-list">
          <li>1つの予定の出欠ではなく、「本祭の日をいつにするか」のように、複数の候補日から都合の良い日・時間帯を出し合うための機能です。</li>
          <li>候補日ごとに用意された時間帯のマス目を、ドラッグ（スマホは指でなぞる）して塗ることで「空いている時間」を申告します。</li>
          <li>誤操作を防ぐため、最初はマス目がロックされています。「入力開始」を押すと編集できるようになり、「完了」を押すと保存されます。ロック中でも、すでに自分が選んだ時間帯は色付きのまま見えます。</li>
          <li>「みんなの結果を見る」に切り替えると、参加できる人数が多い日・時間帯が自動で提案されます。誰が未回答かもここで確認できます。</li>
          <li>都合が合わない・まだ分からない場合は、備考欄に自由記述で書いても構いません。</li>
        </ul>
      </div>
    </section>

    <section class="panel">
      <h2>使ううえで気をつけてほしいこと</h2>
      <ul class="guide-list guide-rules">
        <li><strong>調整系の回答後は一報</strong><br>出欠回答や日程アンケートに入力しただけでは気づいてもらえないことがあります。回答した後はLINEグループにも一言報告してください。</li>
        <li><strong>「未定」はいつまでに分かるか一報</strong><br>「未定」で回答するときは、サイト上の「いつまでに分かるか」欄への入力だけでなく、いつ頃分かりそうかをLINEグループにも一報してください。</li>
        <li><strong>自分で決めた報告期日は守る</strong><br>「未定」のときに自分で設定した「いつまでに分かるか」の期日は必ず守り、期日までに参加／不参加を確定させて回答を更新してください。</li>
        <li><strong>締切を過ぎて予定が変わる場合は若衆と政やに一報</strong><br>回答期限を過ぎるとサイト上では自分で変更できなくなります。締切後にどうしても予定が変わる場合は、サイトでの修正を待たず、若衆メンバーと政や（スタッフ）の両方にLINE等で一報してください。</li>
        <li><strong>締切後の代理変更が終わったら一報＆本人確認</strong><br>締切後の変更は管理者による代理入力で対応します。管理者が代理で変更した後は、その旨を一報するとともに、本人にもサイト上の内容が合っているか確認を取ってください。</li>
      </ul>
    </section>
  `;
}
