...
function staffGuideHtml() {
  return `
	...
	　<li>回答期限を過ぎた場合管理者のみ変更可能となります。</li>
          <li>絞り込み（分類・年月・回答状況など）で見たい予定だけを表示できます。</li>
          <li>この画面はメンバーの出欠状況を確認するためのもので、政やスタッフがここから代理で出欠を入力・変更することはできません（本人か管理者のみ変更できます）。</li>
        </ul>
      </div>
  ...
  return `
    <section class="panel">
      ...
      <div class="panel-inset">
        <h3>出欠回答</h3>
        <ul class="guide-list">
          <li>予定一覧でも回答できてかつ共有された回答リンクから開いた場合はこの専用画面が開く。入力できる項目は予定一覧の回答画面と同様。</li>
      ...
      <div class="panel-inset">
        ...
        <ul class="guide-list">
          ...
          <li>誤操作を防ぐために最初はマス目がロックされており、「入力開始」を押すと編集でき。「完了」を押すと保存される。ロック中でもすでに自分が選んだ時間帯は色付きのまま見える。</li>
