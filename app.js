  ...
  return `
    <section class="panel">
      ...
      <div class="panel-inset">
        <h3>予定一覧</h3>
        <ul class="guide-list">
          <li>若衆全体の予定が日付ごとに並びます。予定名左の帯の色で分類（練習・演奏依頼・たこ焼きなど）が分かります。</li>
...
function memberGuideHtml() {
  return `
    <section class="panel">
      <h2>使い方・見方</h2>
      <p class="muted">各画面の使い方</p>

      <div class="panel-inset">
        <h3>予定一覧</h3>
        <ul class="guide-list">
          <li>若衆全体の予定が日付ごとに並び、予定名左の帯の色で分類（練習・演奏依頼・たこ焼きなど）が分かる。</li>
          <li>見出しをタップすると詳細が開閉され、まだ回答していない予定は最初から開いた状態となっている。</li>
          <li>名前の横にある「参加／不参加／未定／時間限定」ボタンをタップすると回答でき、不参加・未定・時間限定を選ぶと詳細欄が自動で開く。</li>
          <li>タップしただけではまだ保存されず画面右下の「回答完了」ボタンを押して初めてサイトに反映される。押す前に画面を閉じると入力が消える点に注意</li>
        </ul>
      </div>
      ...
      <div class="panel-inset">
        <h3>出欠回答</h3>
        <ul class="guide-list">
          <li>予定一覧でも回答できてかつ共有された回答リンク（LINE等で送られてくるURL）から開いた場合はこの専用画面が開く。入力できる項目は予定一覧の回答画面と同様。</li>
          <li>回答期限が設定されている予定は期限日を過ぎると変更不可（期限日当日23:59までは変更可能）。</li>
        </ul>
      </div>
      ...
      <div class="panel-inset">
        <h3>日程アンケート</h3>
        <ul class="guide-list">
          <li>1つの予定の出欠ではなく複数の候補日から都合の良い日・時間帯を出し合うための機能。</li>
          <li>候補日ごとに用意された時間帯のマス目をドラッグ（スマホは指でなぞる）して塗ることで「空いている時間」を申告可能。</li>
          <li>誤操作を防ぐために最初はマス目がロックされており、「入力開始」を押すと編集できてかつ「完了」を押すと保存される。ロック中でもすでに自分が選んだ時間帯は色付きのまま見えます。</li>
          <li>「みんなの結果を見る」に切り替えると参加できる人数が多い日・時間帯が自動で提案され、誰が未回答かもここで確認可能。</li>
          <li>都合が合わない・まだ分からない場合は備考欄に記入。</li>
        </ul>
      </div>
    </section>
    ...
    <section class="panel">
      ...
      <ul class="guide-list guide-rules">
        <li><strong>調整系の回答後は一報</strong><br>回答した後はLINEグループにも一言報告すること。</li>
        <li><strong>「未定」はいつまでに分かるか一報</strong><br>「未定」で回答するときはサイト上の「いつまでに分かるか」欄への入力だけでなくいつ頃分かりそうかをLINEグループにも必ず一報すること。</li>
        <li><strong>自分で決めた報告期日は守る</strong><br>「未定」のときに自分で設定した「いつまでに分かるか」の期日は必ず守り、期日までに参加／不参加を確定させて回答を更新し、LINEグループで一報すること。</li>
        <li><strong>締切を過ぎて予定が変わる場合は若衆と政やに一報</strong><br>回答期限を過ぎるとサイト上で変更不可となる。締切後にどうしても予定が変わる場合は若衆メンバーと政や（スタッフ）の両方にLINEグループで一報すること。</li>
        <li><strong>締切後の代理変更が終わったら一報＆本人確認</strong><br>締切後の変更は管理者による代理入力で対応。管理者が代理で変更した後はその旨が一報されるがサイト上の内容が合っているか確認すること。管理者からの連絡が無い場合は忘れている可能性があるためメンバー自身が責任をもって催促すること。</li>
      </ul>
    </section>
  `;
}
