...
- 予定の「非表示」機能（2026-08-12追加）：削除とは別軸で`events.visible`
  （boolean、デフォルトtrue）を追加。管理画面の「予定一覧」で予定ごとに
  「非表示にする」/「表示に戻す」ボタンを切り替えられる
  （`toggleEventVisibility()`）。非表示の予定は一般メンバー向けの表示
  （`getFilteredPublicEvents()`＝予定一覧、`populateAnswerEventSelect()`＝
  出欠回答の選択肢、`renderTopHighlights()`の「次の予定」）から除外される。
  管理画面の予定一覧テーブルでも既定では隠れるが、「削除済みも表示する」の
  右隣に追加した「非表示も表示する」チェックボックスで確認・操作できる
  （members.visibleと同じ設計）。
  2026-08-12に追記：1件ずつのトグルだけでなく、「終了した予定をまとめて
  非表示にする」ボタンも追加（`hidePastEventsBulk()`）。`isPastEvent()`で
  開催日が今日より前と判定される予定のうち、削除済み・すでに非表示のものを
  除いた件数を確認ダイアログで示してから、`.in('id', [...])`で一括更新する
