...
- 使い方ページ（`guideView`、上部タブ「使い方」）：2026-08-06に追加。全ロール共通で
  見られる静的な説明ページ。「予定一覧」「出欠回答」「日程アンケート」各画面の
  使い方に加えて、「使ううえで気をつけてほしいこと」として運用ルール
  （調整系の回答後は一報／「未定」はいつまでに分かるか一報／自分で決めた
  報告期日は守る／締切後に予定が変わる場合は若衆と政やに一報／締切後の
  代理変更が終わったら一報＆本人確認）を掲載。DB連携無しの完全な静的HTML
  （`switchView('guide')`で他の画面と同じ仕組みで切り替わるだけ）。
  運用ルールの文面自体はSPEC.md「11. 利用上の留意点」にも同じ内容を記載しており、
  今後ルールを変える時はこの2箇所を両方更新する
