...
| ファイル | 役割 |
|---|---|
| `index.html` | 画面本体（ログイン/登録/トップ/予定一覧/日程調整回答/候補日程調整/予定追加/管理） |
...
- **member（メンバー）**：自分の出欠回答、候補日程調整への入力ができる。
...
- トップページ：Storageの非公開バケット（`top-photos`）から写真を取得し
  ...
  ピル表示（写真を邪魔しない）
- 予定一覧：日ごとに見出し、予定ごとにカード表示（枠線で区切り、視覚的に
  区別しやすい）。各予定の各メンバー欄に参加/不参加/未定/時間限定ボタンが
  直接あり、別画面に遷移せずその場で回答できる。名前をクリックすると
  理由・時間帯などの詳細欄が開閉する。管理者は全メンバー分をその場で
  代理回答でき、予定の「編集」ボタンからそのまま編集画面へ飛べる
- 日程調整回答（レガシー画面）：予定を選んで回答する専用画面。管理者のみ
  「代理入力するメンバー」を選択可能。回答期限を過ぎると一般メンバーは
  編集不可（UIとRLS両方で強制）
- 候補日程調整：特定の1予定の出欠とは別の仕組み。管理者が期間と1日の
  時間帯を決めて作成し、メンバーはGoogleカレンダー風の週グリッドを
  ドラッグして空き時間を申告する（1時間ごとの行、15/30分刻みは色分け
  ブロックで表現）。ドラッグ選択は「完了」ボタンで確定保存。結果表示では
  重なりをヒートマップ表示し「空いている時間帯トップ3」「参加可能人数が
  多い日トップ3」を自動提案。備考欄、管理者代理入力、リセットボタンにも対応
- 予定追加：対象メンバーは全員にチェックボックスがあり自由に絞り込める
  （未変更なら在籍期間から自動判定、日付を変えると自動でプレビューが
