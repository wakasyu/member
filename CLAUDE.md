...
- 日程アンケート：特定の1予定の出欠とは別の仕組み。管理者が「候補日」を
  1日ずつ追加して作成する（`availability_poll_days`テーブル、候補日ごとに
  個別の開始・終了時刻を持てる。日付は連続している必要はなく、飛び石でも
  よい。2026-07-21にperiod_start〜period_end＋1日共通の時間帯という旧モデルから
  この個別候補日モデルへ移行済み）。メンバーはGoogleカレンダー風のグリッドを
...
- **2026-07-18時点でコードは修正・push済みだが、ユーザーの実機（iOS Safari）で
  ...
  - 2026-07-21の日程アンケート再設計（候補日リストUI、日ごとに時間帯が違う
    グリッドの`.poll-cell.unavailable`表示、ページ送り）はブラウザ検証で
    数値・DOM構造は確認済みだが、実機での見た目・操作感は未確認
