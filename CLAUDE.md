...
- 予定の「非表示」機能（2026-08-12追加）：削除とは別軸で`events.visible`
  （boolean、デフォルトtrue）を追加。非表示の予定は一般メンバー向けの表示
  （`getFilteredPublicEvents()`＝予定一覧、`populateAnswerEventSelect()`＝
  出欠回答の選択肢、`renderTopHighlights()`の「次の予定」）から除外される
  （members.visibleと同じ設計）。
  管理画面の「予定一覧」テーブルでは削除済み以外は行ごとの
  「表示」チェックボックス（`data-toggle-event-visible`、change イベントで
  `handleAdminEventsChange()`→`toggleEventVisibility(id, checked)`）で
  直接オン/オフを切り替えられる。当初は「非表示も表示する」チェックボックス
  ＋「終了した予定をまとめて非表示にする」ボタンだった（一括処理は
  `hidePastEventsBulk()`）が、「チェックボックスで表示/非表示を選べる形が
  分かりやすい」というフィードバックを受けて2026-08-12中に上記の行チェック
  ボックス方式へ置き換え、両方とも削除した。
  さらに同日、メンバー向け予定一覧の「終了した予定を表示する」と同じ考え方の
  フィルターを管理画面にも追加（`showPastAdminEvents`チェックボックス、既定は
  非表示＝`!isPastEvent(event)`のみ表示）。「表示」チェックボックスは
  events.visible（公開/非公開）の切り替え、こちらは終了日時によるテーブルの
  絞り込みで、別軸の機能
